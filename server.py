...
def main():
	...
	while servidor.readable:
		readable, writable, exceptional = select.select(servidor.readable, servidor.writable, servidor.readable)
		for s in readable:
			#print("Dentro1")	
			if s is servidor.con:
				...
				if tipoMensagem == 6:
					...
					print("Send ",origem, " ", dados_novos, " para id ", destino) 
		...
		for s in writable:
			try:
				...
				#print(next_msg)
				s.send(next_msg)
				if unpack('!H', next_msg[0:2])[0]==5:
					s.setblocking(1)
					s.settimeout(5)
					try:
						data2 = s.recv(8)
						tipoMensagem2 = unpack("!H", data2[0:2])[0]
						origem2 = unpack("!H", data2[2:4])[0]
						destino2 = unpack("!H", data2[4:6])[0]
						sequencia2 = unpack("!H", data2[6:8])[0]
					except Exception as e:
						print("Excecao "+str(e.__class__.__name__))
						del servidor.connected_sockets[destino]
						servidor.writable.remove(s)
						...
						s.settimeout(0)
						if tipoMensagem2 == 1:
							print("Recebido OK de "+str(origem2))
						else:
							print("Esperava OK")
							servidor.sendErro(origem2, sequencia2)
